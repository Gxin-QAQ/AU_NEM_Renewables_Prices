...
本项目研究澳大利亚国家电力市场（NEM）中风电与集中式光伏出力和批发电价、价格波动及负电价风险的关系。主要分析 NSW1、VIC1、QLD1 和 SA1，TAS1 用于稳健性检验。
...
仓库提供数据处理与估计代码、最终结果表、10 页英文报告和静态看板。原始市场数据不随仓库分发；数据获取与历史验证记录见[复现审计](docs/task11_reproducibility_audit.md)。
...
在对极端占比截尾后，主要样本、燃料分类、固定效应和标准误计算方式的检验保留了上述关联方向。未截尾时，少数 SA1 需求接近零的小时对估计影响过大，电价和负电价系数不再显著。地区差异同样明显：NSW1 的变换价格系数为正，因此总体结果不能直接套用于每个州。
...
研究与网页发布使用同一目录；本地数据、运行环境和私人草稿不上传。详见[项目目录说明](docs/project_layout.md)。
...
天气 IV 未通过识别审查。完整样本的无惩罚 q = 0.50、0.90 和 0.95 分位数模型未在规定时间内完成，因此未报告这些估计，也未改用其他估计量替代。
...
大型市场原始数据不随仓库分发。下载清单记录来源 URL、下载时间、校验和及数据版本，归档清单位于 [`provenance/`](provenance/)，结果汇总表位于 `outputs/tables/`。完整重建需要约 1.98 GB 压缩 AEMO 来源文件。AEMO 和其他来源数据受各自使用条款约束。
...
本仓库中的原创代码和文档采用 [MIT License](LICENSE)。AEMO 与其他第三方来源数据及其相关权利不随本许可授予；使用者仍须遵守各来源的适用条款。
